...
conditionalDescribe('integration tests with real masumi API', () => {
	...
	describe('payment status checking', () => {
		it('should check payment status without errors', async () => {
			...
			if (status) {
				...
				expect(status.onChainState === null || typeof status.onChainState === 'string').toBe(true);
			}
		}, 15000);
	});
	...
	describe('full payment flow (requires actual ADA)', () => {
		...
		it.skip('should complete full payment + purchase flow', async () => {
			...
			console.log('✅ payment created:', paymentResponse.data.blockchainIdentifier?.substring(0, 20) + '...');
			...
			expect(['FundsLocked', 'timeout', 'FundsOrDatumInvalid'].includes(pollResult.status)).toBe(true);
	...
	describe('error handling', () => {
		it('should handle invalid API key gracefully', async () => {
			...
			await expect(createPayment(badConfig, paymentData)).rejects.toThrow(/payment creation failed/);
