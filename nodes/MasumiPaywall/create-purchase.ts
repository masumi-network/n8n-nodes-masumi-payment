...
async function main() {
  ...
  const paymentResponse: PaymentResponse = {
    ...
    data: {
      id: 'test',
      blockchainIdentifier: parsed['blockchain-identifier'] || 'missing_blockchain_identifier',
  ...
  if (!parsed['blockchain-identifier']) {
    ...
    console.error('💡 Usage: ts-node create-purchase.ts --blockchain-identifier <id> --pay-by-time <time> --submit-result-time <time> --unlock-time <time> --external-dispute-unlock-time <time> --input-hash <hash> --identifier <id>');
    process.exit(1);
  }
