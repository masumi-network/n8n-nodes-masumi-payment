 ...
 */
export async function checkPaymentStatus(config: MasumiConfig, paymentIdentifier: string): Promise<PaymentStatus | null> {
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    try {
      ...
      if (status) {
        ...
        if (onChainState && ['FundsOrDatumInvalid', 'RefundRequested', 'Disputed', 'RefundWithdrawn', 'DisputedWithdrawn'].includes(onChainState)) {
